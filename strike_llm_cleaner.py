...
def extract_date_from_header(client: OpenAI, newspaper_header: str, year: str) -> str:
    ...
    print(f"    🗓️  Extracting date from header: {newspaper_header[:50]}...")
    ...
    print(f"    ⚠️  Could not parse date from response: {response}")
    return year
...
Don't write any scripts, just read it. Don't write any accompanying texts like 'here is the data you asked for' just send the plain data."""
    ...
    try:
        ...
        if isinstance(strikes_data, dict):
            ...
            print(f"    ⚠️  Unexpected response format: {type(strikes_data)}")
            return []
        
        print(f"    ✅ Found {len(strikes_data)} strike(s)")
        return strikes_data
...
def process_file(client: OpenAI, input_path: str, output_path: str, input_folder: str) -> bool:
    ...
    try:
        ...
        if not year:
            print(f"    ❌ Skipping file - could not extract year")
            return False
        ...
        if not column_content.strip():
            print(f"    ❌ Skipping file - no column content")
            return False
        ...
        print(f"    ✅ Saved {len(strikes)} strike(s) to: {os.path.basename(output_path)}")
        return True
